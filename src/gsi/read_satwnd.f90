...
!                    call map3grids_m_tm(-1,pflag,presl_thin,nlevp,ntime,dlat_earth,dlon_earth,&
           ...
           cdata_all(27,iout)=hdrdat(9)           ! SWCM, spectral type 1-5
           cdata_all(28,iout)=hdrdat(10)          ! SAZA
           cdata_all(29,iout)=hdrdat(12)          ! SCCF, spectral wavenumber

           if(perturb_obs)then
              cdata_all(30,iout)=ran01dom()*perturb_fact ! u perturbation
              cdata_all(31,iout)=ran01dom()*perturb_fact ! v perturbation
           endif
